...
SELECT EMPNO NO, ENAME ENAME, SAL SALALY FROM EMP;
...
-- 4. 특정 행 출력 : WHERE절(조건절) - 논리연산자 : AND, OR, NOT
    ...
    SELECT ENAME, SAL, SAL*12 ANNUALSAL -- (3순서)
        FROM EMP                        -- (1순서)
        WHERE SAL*12 >= '2400';         -- (2순서) WHERE절에는 필드의 별칭 사용 불가
    -- EX4) 연봉이 3000이상인 직원의 ENAME, SAL, 연봉을 연봉순으로 출력
    SELECT ENAME, SAL, SAL*12 연봉      -- (3순서)
        FROM EMP                        -- (1순서)
        WHERE SAL*12 >= 3000            -- (2순서) WHERE절에는 필드의 별칭 사용 불가
        ORDER BY 연봉;                  -- (4순서) ORDER BY절에는 필드의 별칭 사용 가능
    
    -- EX5 JOB이 MANAGER이거나 10번 부서(DEPTNO) 직원 외의 모든 필드 출력
    SELECT *
        FROM EMP
...
-- 5. 산술연산자(SELECT절, 조건절, ORDER BY절)
    ...
    SELECT ENAME, SAL, COMM, SAL*12+COMM 연봉
        FROM EMP
        ORDER BY 연봉;
        -- 산술 연술의 결과는 NULL을 포함하면 결과도 NULL
    -- NVL(NULL일 수도 있는 필드명), NULL을 대체할 값) - 두 매개변수의 타입이 일치
    SELECT ENAME, SAL, COMM, NVL(COMM, 0), SAL*12+NVL(COMM, 0) 연봉
        FROM EMP;
    -- 모든 사원의 사번, 이름, 상사 사번을 출력 (상사가 없으면 'CEO'로 출력)
    SELECT EMPNO, ENAME, NVL(TO_CHAR(MGR), 'CEO') MGR
        FROM EMP;
...
-- 8. SQL연산자(BETWEEN, IN, LIKE, IS NULL)
    -- (1) BETWEEN A AND B : A부터 B까지(A, B포함, A<=B)
        -- EX1) SAL이 1500이상 3000이하인 직원의 사번, 이름, 급여 출력
        ...
        -- EX3) 이름이 'A','B','C'로 시작하는 직원의 모든 필드를 출력
        SELECT * FROM EMP WHERE ENAME BETWEEN 'A' AND 'D' AND ENAME != 'D' ORDER BY ENAME;
        -- EX4) 82년도에 입사한 직원의 모든 필드를 출력
        SELECT * FROM EMP 
            WHERE TO_CHAR(HIREDATE, 'YY/MM/DD') >= '82/01/01' 
            AND TO_CHAR(HIREDATE, 'YY/MM/DD') <= '82/12/31';
        SELECT * FROM EMP 
            WHERE TO_CHAR(HIREDATE, 'YY/MM/DD') BETWEEN '82/01/01' AND '82/12/31';
    -- (2) 필드명 IN (값1, 값2,.. 값N)
        ...
        -- EX2) 직책(JOB)이 MANAGER거나 ANALYST인 사원의 모든 필드 출력
        SELECT * FROM EMP WHERE JOB IN ('MANAGER','ANALYST');
    -- (3) 필드명 LIKE 패턴 : %(0글자이상), _(한글자)를 포함한 패턴
        ...
        SELECT * FROM EMP WHERE TO_CHAR(HIREDATE, 'YY/MM/DD') LIKE '__/01/__';
    -- (4) 필드명 IS NULL(해당 필드가 널인지 여부)
...
-- ※날짜형(HIREDATE)을 문자형으로 변환 : TO_CHAR(날짜형데이터, 패턴)
        -- YYYY, YY, RR : 년도 / MM : 월 / DD : 일 / DY : 월,화,수 / DAY : 월요일,화요일
        -- HH24, HH12, HH : 시간 / AM이나 PM / MI : 분 / SS : 초
