...
def update_now_report(wb, analysis, summary):
  ...
  if 'prev_report' in sheet_names:
    ...
    print('prev_report 시트를 찾을 수 없어 삭제 명령 실행 불가')
  # 'now_report' 시트 복사
  if 'now_report' in sheet_names:
    ...
    print('now_report 시트를 찾을 수 없어 작업을 중단합니다.')
    wb.close()
    return
  # 분석글(analysis, summary)을 now_report 업데이트
  current_dt = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
  # 시간은 A3셀에 입력
  now_sheet.range('A3').value = current_dt + " 기준" # 오른쪽 정렬
  now_sheet.range('A3').api.HorizontalAlignment = xw.constants.HAlign.xlHAlignRight # = -4152(오른쪽정렬)
  # analysis는 A5셀에 입력
  ...
  now_sheet.api.ExportAsFixedFormat(0, file_path) # 0=pdf, 1=?
  print('pdf 저장 완료 :', file_path)